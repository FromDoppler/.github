...
Sometimes, those seed projects can be outdated or we did not prepare a sed yet, so, you can base your project in [an existing one](https://github.com/orgs/FromDoppler/repositories?q=&type=all&language=&sort=), for example:
...
- [Doppler Session MFE](https://github.com/FromDoppler/doppler-session-mfe) - Minimal microfrontend without UI (based in CRA) with CI and CD to our CDN
...
Knowing exactly what source code corresponds to the system we are runing use to be very useful.
...
In our CDN based micro-frontends, we expose the version in the manifest file:
